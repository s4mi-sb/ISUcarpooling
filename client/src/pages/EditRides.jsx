...
export default function EditRides() {
    ...
    const [timeUpdated,setTime] = useState('');
    ...
    const handleSubmit = async (e)=>{
        ...
        try {
            setLoading(true);
            setError(false);
            const res = await fetch(`/api/rideShare/editRide/${param.rideID}`,{
                method: 'POST',
                headers:{
                    'Content-Type': 'application/json',
                },
                body : JSON.stringify({...formData, userRef: currentUser._id, time:timeUpdated}),
...
return (
    <main className='pt-32'>
      <h1 className='text-4xl text-center my-6 font-semibold underline underline-offset-8'>Edit your Ride</h1>
        ...
        <div className='flex flex-col gap-4 w-full px-6 sm:px-28 lg:px-60'>
            ...
            <div className='flex flex-wrap gap-5'>
                ...
                <div className='gap-2 flex'>
                    ...
                    {/* <input type="checkbox" id='noluggage' className='w-5 rounded-full' onChange={handleChange} checked = {formData.luggage === 'noluggage'}/>
                    <span className='text-sm'>No luggage</span> */}
            ...
            {formData.luggage?<div className='flex items-center gap-5'>
                    ...
                    <input type="number" id='luggageSpace' className='p-3 border border-black rounded-md' min='0' max = '7' onChange={handleChange} value={formData.luggageSpace}/>
            </div>:<div></div>}
            ...
            <button disabled={loading} className='border rounded-md p-3 bg-green-700 text-white hover:opacity-95 disabled:opacity-80'>{loading?'Uploading...':'Done'}</button>
